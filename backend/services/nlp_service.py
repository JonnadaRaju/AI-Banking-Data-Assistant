...
def query_to_sql(user_query: str) -> str:
    logger.info(f"query_to_sql called with USE_OLLAMA={USE_OLLAMA}")
    prompt = build_prompt(user_query)

    if USE_OLLAMA:
        logger.info("Calling _query_ollama")
        try:
            ...
            if not OPENROUTER_API_KEY:
                raise
            logger.warning(
                "Ollama request failed, falling back to OpenRouter. error=%s",
                e,
            )

    if not OPENROUTER_API_KEY:
        raise Exception(
            "No AI provider is configured. Set OPENROUTER_API_KEY or enable a reachable Ollama server."
        )
...
def _query_ollama(prompt: str) -> str:
    ...
    try:
        logger.info(f"Attempting to connect to Ollama at {OLLAMA_BASE_URL} with model {OLLAMA_MODEL}")
        result = client.chat.completions.create(
            model=OLLAMA_MODEL,
            messages=[
                {"role": "system", "content": "Generate one safe PostgreSQL SELECT query only."},
        ...
        sql = clean_sql(raw_sql)
        logger.info(f"SQL generated by Ollama ({OLLAMA_MODEL}): {sql}")
        return sql
